...
import { FaCheck, FaCrown, FaStar, FaRocket, FaCreditCard, FaInfoCircle } from "react-icons/fa";
        ...
        <section className="w-full flex flex-col items-center gap-10">
          <div className="text-center space-y-4">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-primary/10 border border-primary/20 rounded-full mb-1">
              ...
              <span className="text-[10px] font-bold text-primary uppercase tracking-widest">Кредиты</span>
            </div>
            <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight uppercase text-primary-text">Пополнить баланс</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 w-full max-w-5xl">
            {CREDIT_PLANS.map((plan) => {
              ...
              return (
                <div key={plan.id} className={`relative bg-bg-card border rounded-2xl p-6 flex flex-col justify-between gap-6 transition-all duration-300 pastel-shadow-sm hover:pastel-shadow-lg hover:-translate-y-1 ${
                  isPopular ? "border-primary shadow-lg shadow-primary/5 scale-105 ring-2 ring-primary/20" : "border-divider/60"
                }`}>
                  {isPopular && (
                    <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-gradient-to-r from-pink-400 to-rose-400 text-white text-[9px] font-bold uppercase px-3 py-1 rounded-full tracking-wider shadow flex items-center gap-1">
                  ...
                  <div className="space-y-4">
                    <div className="space-y-1">
                      ...
                      <p className="text-2xl font-black tracking-tight text-primary">{plan.price}</p>
                    </div>
                    <div className="text-xs bg-bg-elevated border border-divider/50 p-3 rounded-2xl text-center font-extrabold text-primary">{plan.gens}</div>
                    <p className="text-xs text-secondary-text leading-relaxed font-medium min-h-[2.5rem]">{plan.desc}</p>
                    <ul className="space-y-2 border-t border-divider/40 pt-4 text-xs font-semibold text-secondary-text">
                  ...
                  <button onClick={() => handleCheckout(plan.id)} disabled={loadingPlan !== null}
                    className={`w-full py-3 rounded-full text-xs font-bold transition-all shadow-md cursor-pointer select-none active:scale-[0.98] ${
                      isPopular ? "bg-gradient-to-r from-pink-400 to-rose-400 text-white hover:from-pink-300 hover:to-rose-300 pastel-shadow-sm" : "bg-bg-elevated hover:bg-bg-card-hover text-primary-text border border-divider"
                    }`}>
        ...
        <section className="w-full flex flex-col items-center gap-10">
          <div className="text-center space-y-4">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-primary/10 border border-primary/20 rounded-full">
              ...
              <span className="text-[10px] font-bold text-primary uppercase tracking-widest">Подписка</span>
            </div>
            <h2 className="text-2xl sm:text-3xl font-extrabold tracking-tight uppercase text-primary-text">Оформить подписку</h2>
            <p className="text-xs sm:text-sm text-secondary-text max-w-lg leading-relaxed">
              ...
              return (
                <div key={plan.id} className={`relative bg-bg-card border rounded-2xl p-6 flex flex-col justify-between gap-6 transition-all duration-300 pastel-shadow-sm hover:pastel-shadow-lg hover:-translate-y-1 ${
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="text-xs bg-bg-elevated border border-divider/50 p-3 rounded-2xl text-center font-extrabold text-primary">{plan.gens} / мес</div>
                    <ul className="space-y-2 text-xs font-semibold text-secondary-text">
                      {plan.features.map((f, i) => (
                        <li key={i} className="flex items-center gap-2">
                          <FaCheck className="text-primary text-[10px]" /><span>{f}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                  {isActive ? (
                    <div className="w-full py-3 rounded-full text-xs font-bold text-center bg-primary/10 border border-primary/30 text-primary">✓ Активна</div>
                  ) : (
                    <button onClick={() => handleSubscription(plan.id)} disabled={loadingSub !== null}
                      className={`w-full py-3 rounded-full text-xs font-bold transition-all shadow-md cursor-pointer select-none active:scale-[0.98] ${
                        isPopular ? "bg-gradient-to-r from-pink-400 to-rose-400 hover:from-pink-300 hover:to-rose-300 text-white pastel-shadow-sm" : "bg-bg-elevated hover:bg-bg-card-hover text-primary-text border border-divider"
                      }`}>
